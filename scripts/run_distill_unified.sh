...
TRAIN_DATA="/home/lizixi/xuelang/data/processed/train.json"
EVAL_DATA="/home/lizixi/xuelang/data/processed/dev.json"
OUTPUT_DIR="/home/lizixi/xuelang/output/distill_unified"
MAX_LENGTH=128
BATCH_SIZE=2
GRAD_ACCUM=4
EPOCHS=3
...
if [ ! -d "${TEACHER_MODEL}" ]; then
  echo -e "${RED}[$(timestamp)] 错误: 教师模型目录不存在: ${TEACHER_MODEL}${NC}"
  exit 1
fi

if [ ! -f "${TRAIN_DATA}" ]; then
  echo -e "${RED}[$(timestamp)] 错误: 训练数据文件不存在: ${TRAIN_DATA}${NC}"
  # 尝试使用小型训练集
  SMALL_TRAIN_DATA="/home/lizixi/xuelang/data/processed/small_train.json"
  if [ -f "${SMALL_TRAIN_DATA}" ]; then
    echo -e "${YELLOW}[$(timestamp)] 使用备用训练数据: ${SMALL_TRAIN_DATA}${NC}"
...
if [ ! -f "${EVAL_DATA}" ]; then
  echo -e "${RED}[$(timestamp)] 错误: 验证数据文件不存在: ${EVAL_DATA}${NC}"
  # 尝试使用小型验证集
  SMALL_EVAL_DATA="/home/lizixi/xuelang/data/processed/small_dev.json"
  if [ -f "${SMALL_EVAL_DATA}" ]; then
    echo -e "${YELLOW}[$(timestamp)] 使用备用验证数据: ${SMALL_EVAL_DATA}${NC}"
    EVAL_DATA=${SMALL_EVAL_DATA}
  else
    exit 1
  fi
fi
...
# 词汇表统一设置
export VOCAB_SIZE=151669  # 与Qwen3-8B教师模型词汇表大小一致
...
DEEPSPEED_CMD="deepspeed \
  ...
  /home/lizixi/xuelang/train_ds.py \
  ...
  --fp16 \
  --deepspeed_config /home/lizixi/xuelang/scripts/ds_config.json \
